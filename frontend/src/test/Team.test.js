...
test("test team component rendered", () => {
  ...
  // check executives
  const executives = [
    getByText("Ramon Perez"),
    getByText("Eric Cheng"),
    getByText("Aquib Syed"),
    getByText("Lauren Churchill"),
    getByText("Aayan Rahat"),
    getByText("Evan Fernando"),
  ];
  executives.map((e) => expect(e).toBeInTheDocument());

  const board = [
    getByText("Rishi Ajmera"),
    getByText("Ohik Kwon"),
    getByText("Nolan Du"),
    getByText("Shivam Shah"),
    getByText("Avaneesh Mallela"),
    getByText("Martin Coleman"),
    getByText("Hassan Palanpurwala"),
  ];
